...
/*
 * The same as pgxcrypto_to_base64() below, but does encoding block by
 * block.
 *
 * Note that each processed block is generated by having a carriage return
 * added.
 */
char *
pgxcrypto_to_base64_by_blk(const unsigned char *input, int length)
{
	EVP_ENCODE_CTX *ectx      = EVP_ENCODE_CTX_new();
	StringInfo      b64string = makeStringInfo();
	int             num_blks  = (EVP_ENCODE_CTX_num(ectx) + length) / 48;
	int             outlen;
#define EVP_MAX_FINAL_BUF_SIZE 66
	unsigned char   f_buf[EVP_MAX_FINAL_BUF_SIZE];
	char           *result;

	EVP_EncodeInit(ectx);

	elog(DEBUG2, "num blks = %d", num_blks);

	while (num_blks > 0)
	{
		/*
		 * According to OpenSSL's documentation, blocks and encoded sizes are
		 * calculated like the following, quoted from there:
		 *
		 * "To calculate the required size of the output buffer add
		 *  together the value of inl with the amount of unprocessed data
		 *  held in ctx and divide the result by 48 (ignore any remainder).
		 ...
		 */
		size_t bufsize = ((EVP_ENCODE_CTX_num(ectx) + length) / 48) * 66;
		unsigned char l_buf[bufsize + 1];

		elog(DEBUG2, "base64 encode input block");

		memset(l_buf, '\0', bufsize + 1);
		if (!EVP_EncodeUpdate(ectx, l_buf, &outlen, input, length))
		{
			elog(ERROR, "cannot encode input block to base64");
		}

		if (outlen > 0) {
			appendStringInfoString(b64string, (char *) l_buf);
		}

		num_blks--;
	}

	EVP_EncodeFinal(ectx, f_buf, &outlen);
	appendStringInfo(b64string, (char *)f_buf);

	result = palloc0(b64string->len + 1);
	memcpy(result, b64string->data, b64string->len);
	destroyStringInfo(b64string);
